...
export const visualNovels = pgTable(
  'visual_novels',
  {
    id: uuid('id').defaultRandom().primaryKey(),
  ...
  (table) => [index('idx_vn_status').on(table.status), index('idx_vn_creator').on(table.creatorId)],
);

// ── VN Tags ──────────────────────────────────────────────

export const vnTags = pgTable(
  'vn_tags',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    vnId: uuid('vn_id')
  ...
  (table) => [index('idx_vn_tags_tag').on(table.tag)],
);
...
export const userVNAccess = pgTable(
  'user_vn_access',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id')
...
export const userChapterProgress = pgTable(
  ...
  (table) => [index('idx_chapter_progress_user').on(table.userId, table.chapterId)],
);
...
export const creditTransactions = pgTable(
  ...
  (table) => [index('idx_credit_transactions_user').on(table.userId, table.createdAt.desc())],
);
...
export const creatorEarnings = pgTable(
  ...
  (table) => [index('idx_creator_earnings_status').on(table.creatorId, table.status)],
);
