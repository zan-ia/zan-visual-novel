...
import { checkoutSchema, spendCreditsSchema, CREDIT_PACKAGES, CREATOR_REVENUE_SHARE } from '@zan-vn/shared';
...
creditsRouter.post('/checkout', authenticate, async (req, res) => {
  try {
    ...
    if (!pkg) {
      res.status(400).json({ success: false, error: { statusCode: 400, message: 'Pacote inválido', code: 'INVALID_PACKAGE' } });
      return;
    }

    // In production, this would create a Stripe Checkout Session
    // For now, simulate direct credit addition (MVP mode)
    const [user] = await db.select({ balance: schema.users.creditsBalance }).from(schema.users).where(eq(schema.users.id, req.user!.userId)).limit(1);
    if (!user) {
    ...
    await db.update(schema.users).set({ creditsBalance: newBalance }).where(eq(schema.users.id, req.user!.userId));
    ...
    res.json({ success: true, data: { url: null, credits: newBalance, message: `${pkg.credits} créditos adicionados!` } });
  } catch (err: any) {
    if (err.name === 'ZodError') {
      res.status(400).json({ success: false, error: { statusCode: 400, message: 'Dados inválidos', code: 'VALIDATION_ERROR' } });
      return;
    }
...
creditsRouter.post('/spend', authenticate, async (req, res) => {
  try {
    ...
    if (user.creditsBalance < data.amount) {
      res.status(402).json({ success: false, error: { statusCode: 402, message: 'Créditos insuficientes', code: 'INSUFFICIENT_CREDITS' } });
      return;
    }

    // Get VN creator for revenue share
    const [vn] = await db.select({ creatorId: schema.visualNovels.creatorId }).from(schema.visualNovels).where(eq(schema.visualNovels.id, data.vnId)).limit(1);

    const newBalance = user.creditsBalance - data.amount;

    // Transaction
    await db.transaction(async (tx) => {
      await tx.update(schema.users).set({ creditsBalance: newBalance }).where(eq(schema.users.id, user.id));

      const [transaction] = await tx.insert(schema.creditTransactions).values({
        userId: user.id,
        type: 'spend',
        amount: -data.amount,
      ...
      // Grant access
      await tx.insert(schema.userVNAccess).values({
        userId: user.id,
        vnId: data.vnId,
        hasFullAccess: true,
      }).onConflictDoUpdate({
...
creditsRouter.get('/transactions', authenticate, async (req, res) => {
  const transactions = await db.select().from(schema.creditTransactions).where(eq(schema.creditTransactions.userId, req.user!.userId)).orderBy(sql`${schema.creditTransactions.createdAt} DESC`).limit(50);
