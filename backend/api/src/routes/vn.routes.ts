...
vnRouter.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    const [total] = await db.select({ count: sql<number>`count(*)::int` }).from(schema.visualNovels).where(where);
    const vns = await db.select().from(schema.visualNovels).where(where).orderBy(desc(schema.visualNovels.updatedAt)).limit(pageSize).offset(offset);

    // Get tags for each VN
    const vnIds = vns.map((v) => v.id);
    let tags: Record<string, string[]> = {};
    if (vnIds.length > 0) {
      const tagRows = await db.select().from(schema.vnTags).where(sql`${schema.vnTags.vnId} = ANY(${vnIds})`);
    ...
    const data = await Promise.all(vns.map(async (vn) => {
      const [creator] = await db.select({ displayName: schema.users.displayName, avatarUrl: schema.users.avatarUrl }).from(schema.users).where(eq(schema.users.id, vn.creatorId)).limit(1);
      return { ...vn, creator: creator ?? null, tags: tags[vn.id] ?? [] };
    }));

    res.json({
      success: true,
      data: { data, total: total?.count ?? 0, page, pageSize, totalPages: Math.ceil((total?.count ?? 0) / pageSize) },
    });
  } catch (err: any) {
    if (err.name === 'ZodError') {
      res.status(400).json({ success: false, error: { statusCode: 400, message: 'Parâmetros inválidos', code: 'VALIDATION_ERROR' } });
      return;
    }
...
vnRouter.get('/:id', optionalAuth, async (req, res) => {
  try {
    const [vn] = await db.select().from(schema.visualNovels).where(eq(schema.visualNovels.id, req.params.id)).limit(1);
    if (!vn) {
      res.status(404).json({ success: false, error: { statusCode: 404, message: 'VN não encontrada', code: 'NOT_FOUND' } });
      return;
    }

    const chapters = await db.select().from(schema.chapters).where(eq(schema.chapters.vnId, vn.id)).orderBy(schema.chapters.orderIndex);
    const chapterIds = chapters.map((c) => c.id);

    let scenes: any[] = [];
    let choices: any[] = [];
    if (chapterIds.length > 0) {
      scenes = await db.select().from(schema.scenes).where(sql`${schema.scenes.chapterId} = ANY(${chapterIds})`);
      const sceneIds = scenes.map((s) => s.id);
      if (sceneIds.length > 0) {
        choices = await db.select().from(schema.choices).where(sql`${schema.choices.sceneId} = ANY(${sceneIds})`);
      }
    }

    const chaptersWithScenes = chapters.map((ch) => ({
      ...ch,
      scenes: scenes.filter((s) => s.chapterId === ch.id).map((s) => ({
        ...s,
...
vnRouter.post('/', authenticate, async (req, res) => {
  try {
    ...
    const [vn] = await db.insert(schema.visualNovels).values({
...
vnRouter.patch('/:id', authenticate, async (req, res) => {
  try {
    ...
    await db.update(schema.visualNovels).set({ ...data, updatedAt: new Date() }).where(eq(schema.visualNovels.id, req.params.id));
