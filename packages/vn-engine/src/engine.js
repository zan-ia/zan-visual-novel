...
export class VNEngine {
    ...
    start(story, saveData) {
        ...
        if (saveData) {
            ...
            const firstScene = firstChapter.scenes.find((s) => s.id === firstChapter.startSceneId) ??
                firstChapter.scenes[0];
            if (!firstScene)
                throw new Error('Chapter has no scenes');
    ...
    async generateContinuation() {
        if (!this.llmProvider || !this.story)
            return null;
        const scene = this.getCurrentScene();
    ...
    createSave(slotNumber, label = 'Save') {
        return {
    ...
    generateLLMScene(_currentScene) {
