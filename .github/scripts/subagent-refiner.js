...
function main() {
  ...
  const reviewPattern = trajectory.filter(
    (e) => (e.tool === 'read_file' || e.tool === 'grep_search') && e.input,
  );
  ...
  const testFailures = trajectory.filter(
    (e) => e.tool === 'run_in_terminal' && e.input && e.input.includes('vitest') && e.hasError,
  );
  ...
  if (actions.length === 0) {
    process.stdout.write(
      JSON.stringify({ refined: false, reason: 'No sub-agent changes needed.' }) + '\n',
    );
  } else {
