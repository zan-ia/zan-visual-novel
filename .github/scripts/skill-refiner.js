...
function main() {
  ...
  for (let i = 0; i < trajectory.length - 3; i++) {
    ...
    const hasCreateFile = slice.some(
      (e) => e.tool === 'create_file' && e.input && e.input.includes('route.ts'),
    );
    const hasEdit = slice.some(
      (e) => e.tool === 'edit_file' || e.tool === 'replace_string_in_file',
    );
    const hasTerminal = slice.some(
      (e) => e.tool === 'run_in_terminal' && e.input && e.input.includes('test'),
    );
  ...
  const lintPattern = trajectory.filter(
    (e) =>
      e.tool === 'run_in_terminal' &&
      e.input &&
      (e.input.includes('eslint') || e.input.includes('prettier')),
  );
  ...
  const testPattern = trajectory.filter(
    (e) => e.tool === 'run_in_terminal' && e.input && e.input.includes('npm test'),
  );
  ...
  for (const skill of existingSkills) {
    ...
    if (stats[skill].sessions >= 20 && stats[skill].lastUsed) {
      const daysSinceUsed =
        (Date.now() - new Date(stats[skill].lastUsed).getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceUsed > 30) {
        actions.push({
          operation: 'deprecate',
          skill,
          reason: `Not used in ${Math.floor(daysSinceUsed)} days.`,
        });
  ...
  if (actions.length === 0) {
    process.stdout.write(
      JSON.stringify({ refined: false, reason: 'No skill changes needed.' }) + '\n',
    );
  } else {
