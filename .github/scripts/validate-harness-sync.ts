  ...
  for (const line of yamlBlock.split('\n')) {
    ...
    if (topMatch && !line.startsWith(' ') && !line.startsWith('\t')) {
      ...
      if (currentKey === 'handoffs') {
        inHandoffs = true;
        if (currentList.length > 0 && currentKey !== 'tools' && currentKey !== 'agents') {
          // flush previous
        }
        continue;
      }
      ...
      if (currentKey === 'name') result.name = value.replace(/^["']|["']$/g, '');
      else if (currentKey === 'user-invocable') result.user_invocable = value === 'true';
...
function isValidTool(tool: string): boolean {
  ...
  if (tool.endsWith('/*')) {
    ...
    if (VALID_TOOL_PREFIXES.some((p) => p.startsWith(prefix) || prefix.startsWith(p))) return true;
  }
  return false;
}
...
function validateAgent(filePath: string, fm: AgentFrontmatter): Violation[] {
  ...
  if (fm.agents !== undefined && fm.agents.length === 0 && fm.handoffs && fm.handoffs.length > 0) {
    violations.push({
      agent: agentName,
      severity: 'critical',
...
function main() {
  ...
  if (allViolations.length === 0) {
    console.error(`✅ Harness sync check passed — ${agentCount} agents validated, 0 violations.`);
  } else {
    ...
    console.error(`\n${'─'.repeat(60)}`);
    console.error(`Total: ${criticals.length} critical, ${warnings.length} warnings`);
  }
